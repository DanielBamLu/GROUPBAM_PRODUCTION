<script>
    ...
    let pagesCounter = Math.ceil( ( data.companyArray.length / 10 ) );
    ...
    const search = () => {
        ...
        if ( value != '' )
        {
            for ( let index in data.companyArray )
            {
                if ( data.companyArray[ index ].slug.toLowerCase().includes( value.toLowerCase() ) )
                {
                    searchArray.push( data.companyArray[ index ] );
                }
            }
...
<svelte:head>
    <title>{getTranslatedTextByCode( 'CompanyButton' )}</title>
</svelte:head>
<div class="admin-list">
    ...
    <div class="admin-list-pages">
        {#if isSearch }
                                ...
                                {#if counter >= outerCounter && counter < outerCounter + 10}
                                    <div class="admin-list-item">
                                        ...
                                        <div class="admin-list-item-controler">
                                            <a class="admin-list-item-edit" href="/admin/company/edit/{item.id}">
                                                <Button filled>
            ...
            {/if}
        {:else}
            {#each data.companyArray as company, outerCounter}
                {#if outerCounter % 10 === 0}
                    <div class="admin-list-page {( outerCounter + 10 ) == currentPage*10 ? 'selected' : ''}">
                        {#each data.companyArray as company, counter}
                            {#if counter >= outerCounter && counter < outerCounter + 10}
                                <div class="admin-list-item">
                                    <div class="admin-list-item-title">
                                        {company.name}
                                    ...
                                    <div class="admin-list-item-controler">
                                        <a class="admin-list-item-edit" href="/admin/company/edit/{company.id}">
                                            <Button filled>
                                                <EditIcon data="bright"/>
                                            </Button>
                                        </a>
